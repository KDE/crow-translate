...
void GuiFrontend::prepareEnvironment()
{
...
std::unique_ptr<QCoreApplication> GuiFrontend::createApplication(int &argc, char **argv)
{
    // SingleApplication rather than a plain QApplication: a second launch
    // hands its arguments to the running instance instead of starting a
    // rival one.
    return std::make_unique<SingleApplication>(argc, argv);
}

int GuiFrontend::run(QCoreApplication &app)
{
    Q_UNUSED(app)
    ...
    if (QDBusConnection::sessionBus().isConnected()) {
        ...
        if (QDBusConnection::sessionBus().registerService(service)) {
            ...
            registerDBusObject(m_window.get());
            registerDBusObject(m_window->ocr());
        } else {
